/// A savings account 
pub struct SavingsAccounnt {
    balance: i32,

}

impl SavingsAccounnt {
    /// Create a 'SavingsAccounnt' with a balance of 0
    /// 
    /// # Examples
    /// 
    /// ```
    ...
    pub fn transfer(&self, acc_number:u32, amount:i32)-> Result<String,String>{
        Ok(format!("Transfer ${amount} to ${acc_number}"))
    }
}
...
mod tests {
    ...
    fn should_be_able_to_deposit(){
        let mut account = SavingsAccounnt::new();
        account.deposit(100);
    ...
    fn should_transfer_money() -> Result<(), String> {
        let mut account = SavingsAccounnt::new();
        account.deposit(100);
        account.transfer(123456, 100)?;//? --> to propigate error, no need to explicit the error(yo me entiendo)
        Ok(())
    }
}